...
window.onload=function(){
	...
	div.addEventListener('touchmove', function(event) {
		...
		if (event.targetTouches.length == 1) {
			...
			if(touch.pageX>=($(window).width()-50)){
				div.style.left = $(window).width()-50 + 'px';
				div.style.top = touch.pageY + 'px';
			}else if(touch.pageY>=($(window).height()-50)){
				div.style.left = touch.pageX + 'px';
				div.style.top = $(window).height()-50 + 'px';
			}else {
				div.style.left = touch.pageX + 'px';
				div.style.top = touch.pageY + 'px';
			}
			if(touch.pageY>=($(window).height()-50) && touch.pageX>=($(window).width()-50)){
				div.style.left = $(window).width()-50 + 'px';
				div.style.top = $(window).height()-50 + 'px';
			}
		}
